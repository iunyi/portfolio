...
We would also need to get the position of the cursor movement using `event.pageX` and `event.pageY` and apply those coordinates to the `top` and `left` properties of the cursor with JavaScript:
